...
export function ScreensTable({ screens }: { screens: ScreenRow[] }) {
  const [rows, setRows] = useState(screens);
  ...
  const remove = async (id: string) => {
    ...
    const result = await deleteScreenAction(id);
    setPendingId(null);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-display text-3xl tracking-wide">Screens</h1>
        <Link
            ...
            {rows.length === 0 ? (
              ...
              rows.map((s) => (
                <tr key={s.id} className="bg-bg-card">
                  ...
                  <td className="px-4 py-3 text-text-muted">{TYPE_LABEL[s.type]}</td>
                  <td className="px-4 py-3 text-text-muted font-mono">{s.capacity}</td>
